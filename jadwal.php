...
<head>
    ...
    <title>Monitoring Perangkat IT</title>
        ...
        // Mendapatkan id_maintenance dari query string
        $id_maintenance = isset($_GET['id_maintenance']) ? intval($_GET['id_maintenance']) : 0;

        // Update status notifikasi menjadi 0
        if ($id_maintenance > 0) {
        mysqli_query($koneksi, "UPDATE jadwal_maintenance SET notifikasi = 0 WHERE id_maintenance = $id_maintenance");
}
        $no = 1;
